...
class PokemonFactory {
    ...
    public static generateGymPokemon(gym: Gym, index: number): BattlePokemon {
        ...
        const catchRate: number = this.catchRateHelper(basePokemon.catchRate);
        return new BattlePokemon(pokemon.name, basePokemon.id, basePokemon.type1, basePokemon.type2, pokemon.maxHealth, pokemon.level, catchRate, exp, new Amount(0, GameConstants.Currency.money), shiny, GameConstants.GYM_GEMS, gender, shadow, false, EncounterType.trainer);
    }
    ...
    public static generateDungeonTrainerPokemon(pokemon: GymPokemon, chestsOpened: number, baseHealth: number, level: number, isBoss: boolean, trainerPokemon = 1): BattlePokemon {
        ...
        return new BattlePokemon(name, basePokemon.id, basePokemon.type1, basePokemon.type2, maxHealth, level, catchRate, exp, new Amount(money, GameConstants.Currency.money), shiny, GameConstants.DUNGEON_GEMS, gender, shadow, false, EncounterType.trainer, undefined, ep);
    }
    ...
    public static generateTemporaryBattlePokemon(battle: TemporaryBattle, index: number): BattlePokemon {
        ...
        const gender = this.generateGender(basePokemon.gender.femaleRatio, basePokemon.gender.type);
        const shadow = pokemon.shadow;
        return new BattlePokemon(pokemon.name, basePokemon.id, basePokemon.type1, basePokemon.type2, pokemon.maxHealth, pokemon.level, catchRate, exp, new Amount(0, GameConstants.Currency.money), shiny, GameConstants.GYM_GEMS, gender, shadow, false, encounterType);
    }
    ...
    public static generateAlpha(chance: number, skipBonus = false): boolean {
        ...
        if (Rand.chance(chance / bonus) && player.region == GameConstants.Region.hisui) {
            return true;
        }
        return false;
    }
