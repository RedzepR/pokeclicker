...
class DefeatPokemonsQuest extends Quest implements QuestInterface {
    ...
    private static calcReward(killsNeeded: number, route: number, region: number): number {
        const attacksPerPokemon = Math.ceil(Math.min(4, PokemonFactory.routeHealth(route, region) /
            Math.max(1, App.game.party.calculatePokemonAttack(undefined, undefined, undefined, region, true, undefined, undefined, true, false, Routes.getSubregionByRoute(region, route)))));
